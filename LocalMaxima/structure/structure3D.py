...
class structure(object):
    ...
    def report_gaps(self,aa_present):
        ...
        # detects breaks in a sequence of integers.
    ...
    def min_max(self):
        """ This could be deprecated as min_max_se is more general purpose
        This min_max works for all atoms in the PDB/CIF file, whereas 
        min_max_se works for any structural expression structure's subset.
        """
        for i in self.structure.get_models():
            atoms = []
            for j in i.get_atoms():
                atoms.append(j.get_coord())
            x,y,z = zip(*atoms)
            ...
            print(self.structure.get_id(),'Model',i.get_id(),
                  'Number of Atoms ', len(atoms))
            print('  Xmin - Xmax', Xmin, ' - ', Xmax)
            print('  Ymin - Ymax', Ymin, ' - ', Ymax)
            print('  Zmin - Zmax', Zmin, ' - ', Zmax)
...
all models because it depends on coordinates.")
    ...
    def get_se_output_DEPRECATED(self, optionsse, optionsout):
        se = SE.structural_expression(optionsse)
        # TODO Placing the structure in a dicitionary could be turned in to 
        # a function.
        Entities_in_structure = {}
        Entity_atom_count = 1
        for j in self.structure.get_models():
            Entities_in_structure[j.get_id()] = {}
            for k in j.get_chains():
                Entities_in_structure[j.get_id()][k.get_id()] = {}
                for l in k.get_residues():
                    Entities_in_structure[j.get_id()][k.get_id()][l.get_id()[1]] = []
                    for m in l.get_atoms():
                        Entities_in_structure[j.get_id()][k.get_id()][l.get_id()[1]].append((m.get_id(),Entity_atom_count))
                        Entity_atom_count += 1
        selected_atoms = se.atom_list(Entities_in_structure)
        s2 = struct.Structure.Structure('Structure extracted with SE')
        track_entities = {'model':999,'chain':'&','residue':'&'}
        add_entities = {'model':False,'chain':False,'residue':False}
        for i in selected_atoms:
            for j in self.structure.get_models():
                if j.get_id() == i[0]:
                    if track_entities['model'] == 999:
                        track_entities['model'] = j.get_id()
                        add_entities['model'] = True
                        m2 = struct.Model.Model(j.get_id())
                    elif j.get_id() != track_entities['model']:
                        track_entities['model'] = j.get_id()
                        add_entities['model'] = True
                        m2 = struct.Model.Model(j.get_id())
                    for k in j.get_chains():
                        if k.get_id() == i[1]:
                            if track_entities['chain'] == '&':
                                track_entities['chain'] = k.get_id()
                                add_entities['chain'] = True
                                c2 = struct.Chain.Chain(k.get_id())
                            elif k.get_id() != track_entities['chain']:
                                track_entities['chain'] = k.get_id()
                                add_entities['chain'] = True
                                c2 = struct.Chain.Chain(k.get_id())
                            for l in k.get_residues():
                                if l.get_id()[1] == i[2]:
                                    r2 = struct.Residue.Residue(l.get_id(),l.get_resname(),l.get_segid())
                                    for m in l.get_atoms():
                                        if m.get_id() == i[4]:
                                            r2.add(m)
                                    c2.add(r2)
                            if add_entities['chain']:
                                m2.add(c2)
                                add_entities['chain'] = False
                    if add_entities['model']:
                        s2.add(m2)
                        add_entities['model'] = False
        io = struct.PDBIO()
        io.set_structure(s2)
        io.save(optionsout)

    #def PDBs_from_CIF(self, optionsmodels, optionschains, optionsgroups):
    #    if optionsmodels and optionschains:
    #        print("Extraction Error: Both --chains and --models can't be present at the same time.")
    #        print("                  Extracting by model and chain could generate too many similar files in some cases.")
    #        print("                  The suggested practice is to extract first all models, and then the specific chains")
    #        print("                  from the desired model.")
    #        print("                  Program will exit without results.")
    #        sys.exit(1)
    #    if optionsmodels:
    #        if optionsgroups:
    #            print("ERROR: Option --groups is not allowed with --models.")
    #            print("       Extract models into PDB files and then extract chains from desired models.")
    #            print("       Program will exit without results.")
    #            sys.exit(1)
    #        for i in self.structure.get_models():
    #            self.save_substructure(i, self.file_name + "_" + str(i.get_id()) + ".pdb")
    #    if optionschains:
    ...
    #                    if j.get_id() in h:
    #                        m2.add(j)
    #                self.save_substructure(s2, self.file_name + "_" + str(i.get_id()) + "_" + h + ".pdb")
    ...
    def fix_pdb_from_CHARMM(self,a=22, b=73):
        ...
        PDB_file = self.dir_path+'/'+self.file_name+'.'+self.file_sufix
        if not os.path.exists(PDB_file):
            print("Error: No valid --input option entered. Type -h or --help.")
            print("       PDB input file path:"+PDB_file)
            sys.exit(1)
...
#sp = pdb_parser.get_structure("pdb",strt_path+'1brs_comp_1rr.pdb')
# This fixes the problem of CHARMM not generating a chain id.
# But needs to be tested in more systems
...
#prot_str.add_sequence('fasta', sqnc_path+'1brs.fasta.txt')
#prot_str.summary()
# get the gap resort for a structure with a sequence
#prot_str.gap_report()
# get gap_report for a structure without sequence
#prot_str = structure(strt_path+'1brs.pdb')
#prot_str.gap_report()
# gap report from a CIF structure file
#prot_str = structure(strt_path+'1brs.cif')
#prot_str.gap_report()
##############################################
...
#a = "m[0];c[A];r[3:6];a[CA,CG]&m[0];c[C];r[8,10];a[*]"
#prot_str.atom_list(a)
